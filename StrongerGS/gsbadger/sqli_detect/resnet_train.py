...
#读取正负样本
p_sample_label = []
f = open("data\\p_sample.txt",encoding='utf-8') #加载正样本
while True:
    line = f.readline()
    if line:
        line = line.split('\t')[1]
        label_c = [1]
        sample_label = [] 
        sample_label.append(line)
        sample_label.append(label_c)
        p_sample_label.append(sample_label)
    else:
        break
f.close()   
n_sample_label = []
f = open("data\\n_sample.txt",encoding='utf-8')  #加载负样本
while True:
    line = f.readline()
    if line:
        line = line.split('\t')[1]
        ...
        sample_label.append(line)
        sample_label.append(label_c)
        n_sample_label.append(sample_label)
    else:
        break
f.close()

t_sample_label = p_sample_label + n_sample_label
...
for epoch in range(EPOCH):
    ...
    fc_loss_all = 0.0
    optimizer = optim.Adam(itertools.chain(mymodel.parameters(),), lr=0.001,  weight_decay=2e-4)
    ...
    random.shuffle(t_sample_label)
    
    for i in range(len(t_sample_label)):
        sentence = t_sample_label[i][0]
        label = torch.tensor(t_sample_label[i][1]).reshape(1)
    ...
    print('\nEpoch: %d finish' % (epoch + 1))

torch.save(mymodel.state_dict(), 'resnet18.pth')  #保持预训练好的resnet模型
