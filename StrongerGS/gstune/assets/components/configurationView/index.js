...
/*      ['DB Version', dbVersion],*/
  ...
  const renderConfigResult = (codeHighlightStyle) => {
    ...
    return (
      <React.Fragment>
        {isAlterSystem ?
          <p><strong>ALTER SYSTEM</strong> 在数据库中用ALTER SYSTEM SET语句来写入这些参数</p> :
          <p>在<strong>postgresql.conf</strong>配置文件中修改或添加如下参数并重启数据库</p>}
        ...
        <div className="configuration-view-copy-wrapper">
          <CopyButton className="configuration-view-copy-button"
            text={generatedConfigVal} label="复制配置" />
        </div>
      </React.Fragment>
    )
  }
