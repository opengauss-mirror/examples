...
function redis_connection()
   local redis_con = {}
   redis_con[0] = redis.connect('127.0.0.1', 6379)
   redis_con[1] = redis.connect('127.0.0.1', 6389)
   redis_con[2] = redis.connect('127.0.0.1', 6399)
   redis_con[3] = redis.connect('127.0.0.1', 6309)
   for i=0, 3 do
       local response = redis_con[i]:ping()
       if not response then
           print("failed to connect redis".. i)
           return false,nil
       end
   end
   return true, redis_con
end

function redis_close(redis_con)
   for i=0, 3 do
       redis_con[i]:close()
   end
end

function redis_cleanup(redis_con)
   for i=0, 3 do
       redis_con[i]:flushall()
   end
end
...
function cmd_prepare()
   ...
   local res, redis_con = redis_connection()
   if not res then
   ...
   sysbench.opt.threads do
     create_table(drv, con, redis_con, i)
   end
end
...
function create_table(drv, con, redis_con, table_num)
...
CREATE TABLE sbtest%d(
  ...
  %s (id)
   ...
   for i = 1, sysbench.opt.table_size do
      ...
      redis_key = table_num .. ":" .. i
      redis_value = i .. "," .. k_val .. "," .. c_val .. "," .. pad_val
 
      local redis_idx = i % 4
      redis_con[redis_idx]:set(redis_key, redis_value)      
   end
...
function thread_init()
   ...
   res, redis_con = redis_connection()
   if not res then
       print("failed to connect redis")
       return
   end
...
function execute_point_selects()
   ...
   for i = 1, sysbench.opt.point_selects do
      ...
      local redis_idx = id % 4
      value = redis_con[redis_idx]:get(tnum .. ":" .. id)

      if value == nil then
          stmt[tnum].point_selects:execute()
          --print("get data from db, redis_idx is " ..  redis_idx .. ", key is " .. tnum .. ":" .. id)
      else
          stmt[tnum].point_selects:counter_inc(1)
      end
   end
end
...
function execute_index_updates()
   ...
   for i = 1, sysbench.opt.index_updates do
      id = get_id()
      param[tnum].index_updates[1]:set(id)
      ...
      value = redis_con[redis_idx]:get(tnum .. ":" .. id)
      if value ~= nil then
          local sp = split(value, ',')
          local k = tonumber(sp[2]) + 1
          local new_value = sp[1] .. "," .. k .. "," .. sp[3] .. "," ..sp[4]
          redis_con[redis_idx]:set(tnum .. ":" .. id, new_value)
      end
   end
end
...
function execute_non_index_updates()
   ...
   for i = 1, sysbench.opt.non_index_updates do
      id = get_id()
      local c_val = sb_rand_str(c_value_template)
      ...
      if value ~= nil then
          local sp = split(value, ',')
          local new_value = sp[1] .. "," .. sp[2] .. "," .. c_val .. "," ..sp[4]
          redis_con[redis_idx]:set(tnum .. ":" .. id, new_value)
      end  
   end
end
...
function execute_delete_inserts()
   ...
   for i = 1, sysbench.opt.delete_inserts do
      ...
      local redis_idx = id % 4
      redis_con[redis_idx]:del(tnum .. ":" .. id)
      
      local c_val = sb_rand_str(c_value_template)
      local pad_val = sb_rand_str(pad_value_template)
      param[tnum].inserts[1]:set(id)
      param[tnum].inserts[2]:set(k)
      param[tnum].inserts[3]:set(c_val)
      param[tnum].inserts[4]:set(pad_val)
      stmt[tnum].inserts:execute()
      local new_value = id .. "," .. k .. "," .. c_val .. "," .. pad_val
      redis_con[redis_idx]:set(tnum .. ":" .. id, new_value)
   end
end
