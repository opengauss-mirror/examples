 ...
 * @Description
 */
public abstract class RTNode implements Serializable
{
    /**
     * 结点所在的树
     */
...
//		}
        ...
        int c = total/2 + 1;
        //每个结点最小条目个数
        int nodeCapacity = rtree.getNodeCapacity();
        int minNodeSize = Math.round( nodeCapacity* rtree.getFillFactor());
        //至少有两个
        if(minNodeSize < 2)
            minNodeSize = 2;
    ...
    public List<Rectangle> getObjects() {
        List<Rectangle> d = new ArrayList<>();
        Rectangle[] datas = getDatas();
        for (Rectangle sRectangle:datas)
            d.add(sRectangle);
