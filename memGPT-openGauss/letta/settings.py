...
class Settings(BaseSettings):
    ...
    # OpenGauss vector database configuration (uses same DB as main storage)
    enable_opengauss: bool = Field(default=False, description="Enable OpenGauss vector storage")
    # Map OpenGauss settings to standard PG environment variables
    opengauss_host: str = Field(default="localhost", description="OpenGauss host", validation_alias=AliasChoices("letta_pg_host", "letta_opengauss_host"))
    opengauss_port: int = Field(default=5432, description="OpenGauss port", validation_alias=AliasChoices("letta_pg_port", "letta_opengauss_port"))
    opengauss_database: str = Field(default="letta", description="OpenGauss database name", validation_alias=AliasChoices("letta_pg_db", "letta_opengauss_database"))
    opengauss_username: str = Field(default="postgres", description="OpenGauss username", validation_alias=AliasChoices("letta_pg_user", "letta_opengauss_username"))
    opengauss_password: Optional[str] = Field(default=None, description="OpenGauss password", validation_alias=AliasChoices("letta_pg_password", "letta_opengauss_password"))
    opengauss_table_name: str = Field(default="passage_embeddings", description="OpenGauss embeddings table name", validation_alias=AliasChoices("letta_pg_table_name", "letta_opengauss_table_name"))
    opengauss_ssl_mode: str = Field(default="prefer", description="OpenGauss SSL mode", validation_alias=AliasChoices("letta_pg_ssl_mode", "letta_opengauss_ssl_mode"))
    ...
    def letta_pg_uri(self) -> str:
        ...
        if self.pg_uri:
            ...
            return f"postgresql+pg8000://{self.pg_user}:{encoded_password}@{self.pg_host}:{self.pg_port}/{self.pg_db}"
        else:
