    ...
    try:
        ...
        def patched_get_server_version_info(self, connection):
            try:
                ...
                if 'opengauss' in error_str or 'version' in error_str or 'string' in error_str:
                    print(f"[ALEMBIC PATCH] OpenGauss version detection error: {e}")
                    print("[ALEMBIC PATCH] Using PostgreSQL 13 version tuple for OpenGauss compatibility")
                    return (13, 0)
                else:
                    print(f"[ALEMBIC PATCH] Non-OpenGauss error, re-raising: {e}")
                    raise
...
if settings.letta_pg_uri_no_default:
    ...
    escaped_uri = settings.letta_pg_uri.replace('%', '%%')
