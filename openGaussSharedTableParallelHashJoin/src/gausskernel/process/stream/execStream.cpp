...
static StreamSharedContext* buildLocalStreamContext(Stream* streamNode, PlannedStmt* pstmt)
{
    ...
    sharedContext->parallel_state=(ParallelHashJoinState*)palloc0(sizeof(ParallelHashJoinState));
    sharedContext->parallel_state->build_barrier=(myBarrier *)palloc0(sizeof(myBarrier));
    myBarrierInit(sharedContext->parallel_state->build_barrier,producerNum);
    sharedContext->parallel_state->batch_barrier=(myBarrier *)palloc0(sizeof(myBarrier));
    myBarrierInit(sharedContext->parallel_state->batch_barrier,producerNum);

    sharedContext->parallel_state->grow_batches_barrier=(myBarrier *)palloc0(sizeof(myBarrier));
    myBarrierInit(sharedContext->parallel_state->grow_batches_barrier,0);
    sharedContext->parallel_state->grow_buckets_barrier=(myBarrier *)palloc0(sizeof(myBarrier));
    myBarrierInit(sharedContext->parallel_state->grow_buckets_barrier,0);

    sharedContext->parallel_state->memoryspinlock=(pthread_spinlock_t*)palloc0(sizeof(pthread_spinlock_t));
    pthread_spin_init(sharedContext->parallel_state->memoryspinlock,PTHREAD_PROCESS_PRIVATE);
    sharedContext->parallel_state->chunkspinlock=(pthread_spinlock_t*)palloc0(sizeof(pthread_spinlock_t));
    pthread_spin_init(sharedContext->parallel_state->chunkspinlock,PTHREAD_PROCESS_PRIVATE);  
    sharedContext->parallel_state->parallelchunks=NULL;
    sharedContext->parallel_state->spaceUsed=0;
    sharedContext->parallel_state->totalTuples=0;
    sharedContext->parallel_state->paralleloldchunks=NULL;
    sharedContext->parallel_state->sharebatchCxt=AllocSetContextCreate(localStreamMemoryCtx,
        "shareHashBatchContext",
        ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE,
        SHARED_CONTEXT);
    ...
    MemoryContextSwitchTo(localStreamMemoryCtx);
