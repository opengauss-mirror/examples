...
static void* parallel_dense_alloc(HashJoinTable hashtable, Size size);
static void ExecParallelHashIncreaseNumBuckets(HashJoinTable hashtable);
static void ExecParallelHashIncreaseBuckets(HashJoinTable hashtable);
/* ----------------------------------------------------------------
 *		ExecHash
 *
...
{
    if(node->parallel_state)
        return MultiExecParallelHash(node);
    else
        return MultiExecPrivateHash(node);
}
Node* MultiExecParallelHash(HashState* node){
    ...
    myBarrier    *build_barrier;
    myBarrier    *grow_batches_barrier;
    myBarrier    *grow_buckets_barrier;
	...
	Assert(myBarrierPhase(build_barrier) >= PHJ_BUILD_ALLOCATE);
    switch (myBarrierPhase(build_barrier))
	{
		case PHJ_BUILD_ALLOCATE:
			 ...
			 */
			myBarrierArriveAndWait(build_barrier);
            hashtable->buckets.shared = pstate->shared;

			/* Fall through. */

		case PHJ_BUILD_HASH_INNER:
            myBarrierAttach(pstate->grow_batches_barrier);
            myBarrierAttach(pstate->grow_buckets_barrier);
            myBarrierArriveAndWait(pstate->build_barrier);
            /*
            ...
            */
            ...
            for (;;) {
                ...
                if (ExecHashGetHashValue(hashtable, econtext, hashkeys, false, hashtable->keepNulls, &hashvalue)) {
                    ...
                    hashtable->parallel_state->totalTuples+=1;
                }
            }
            myBarrierArriveAndWait(pstate->grow_batches_barrier);
            myBarrierDetach(pstate->grow_batches_barrier,false);
            ...
            myBarrierArriveAndWait(pstate->grow_buckets_barrier);
            myBarrierDetach(pstate->grow_buckets_barrier,false);
            ...
            hashtable->parallel_state->spaceUsed += hashtable->nbuckets * sizeof(HashJoinTuple);
            if (hashtable->parallel_state->spaceUsed > hashtable->spacePeak)
                hashtable->spacePeak = hashtable->parallel_state->spaceUsed;
            ...
            /* must provide our own instrumentation support */
            if (node->ps.instrument) {
                InstrStopNode(node->ps.instrument, hashtable->parallel_state->totalTuples);
            ...
            hashtable->log2_nbuckets=pstate->log2_nbuckets;
            ...
            if(hashtable->nbatch<pstate->nbatch){
                ...
                if (hashtable->innerBatchFile == NULL) {
                    ...
                    hashtable->innerBatchFile = (BufFile**)palloc0(pstate->nbatch * sizeof(BufFile*));
                    hashtable->outerBatchFile = (BufFile**)palloc0(pstate->nbatch * sizeof(BufFile*));
                    /* time to establish the temp tablespaces, too */
                    PrepareTempTablespaces();
                } else {
                    /* enlarge arrays and zero out added entries */
                    hashtable->innerBatchFile = (BufFile**)repalloc(hashtable->innerBatchFile, pstate->nbatch * sizeof(BufFile*));
                    hashtable->outerBatchFile = (BufFile**)repalloc(hashtable->outerBatchFile, pstate->nbatch * sizeof(BufFile*));
                    errno_t rc = memset_s(hashtable->innerBatchFile + hashtable->nbatch,
                        (pstate->nbatch - hashtable->nbatch) * sizeof(BufFile*),
                        0,
                        (pstate->nbatch - hashtable->nbatch) * sizeof(BufFile*));
                    securec_check(rc, "\0", "\0");
                    rc = memset_s(hashtable->outerBatchFile + hashtable->nbatch,
                        (pstate->nbatch - hashtable->nbatch) * sizeof(BufFile*),
                        0,
                        (pstate->nbatch - hashtable->nbatch) * sizeof(BufFile*));
                    securec_check(rc, "\0", "\0");
...
Node* MultiExecPrivateHash(HashState* node){
    PlanState* outerNode = NULL;
    List* hashkeys = NIL;
    HashJoinTable hashtable;
    TupleTableSlot* slot = NULL;
...
HashState* ExecInitHash(Hash* node, EState* estate, int eflags)
{
    ...
    if(u_sess->stream_cxt.producer_dop > 1){
        hashstate->parallel_state=u_sess->stream_cxt.producer_obj->getSharedContext()->parallel_state;
    }
    /*
 ...
 * ----------------------------------------------------------------
 */
HashJoinTable ExecHashTableCreate(Hash* node, List* hashOperators, bool keepNulls, List *hash_collations, ParallelHashJoinState* parallel_state)
{
    HashJoinTable hashtable;
    Plan* outerNode = NULL;
    int nbuckets;
    ...
    hashtable->buckets.unshare= NULL;
    ...
    hashtable->parallel_state=parallel_state;
    if(parallel_state){
        hashtable->parallelchunks=parallel_state->parallelchunks;
        hashtable->sharebatchCxt=parallel_state->sharebatchCxt;
        hashtable->smp_id=u_sess->stream_cxt.smp_id;
        hashtable->producer_dop=u_sess->stream_cxt.producer_dop;

        parallel_state->nbuckets = nbuckets;
        parallel_state->log2_nbuckets = log2_nbuckets;
        parallel_state->nbatch = nbatch;
    }
    /*
    ...
    if(hashtable->parallel_state){
		...
		myBarrier    *build_barrier=pstate->build_barrier;
        if(myBarrierPhase(build_barrier) == PHJ_BUILD_ELECT &&myBarrierArriveAndWait(build_barrier)){
            MemoryContextSwitchTo(pstate->sharebatchCxt);
            pstate->shared=(std::atomic<HashJoinTupleData*>*)palloc0(nbuckets * sizeof(std::atomic<HashJoinTupleData*>));
            hashtable->buckets.shared = pstate->shared;
            for(int i=0;i<nbuckets;i++){
                hashtable->buckets.shared[i]=nullptr;
            }
            MemoryContextSwitchTo(oldcxt);
        }
    }else{
        ...
        */
        ...
        hashtable->buckets.unshare = (HashJoinTuple*)palloc0(nbuckets * sizeof(HashJoinTuple));//分配内存空间
 ...
 */
void ExecParallelHashIncreaseNumBatches(HashJoinTable hashtable)
{
    ...
    errno_t rc;
    ParallelHashJoinState *pstate = hashtable->parallel_state;
    myBarrier    *grow_batches_barrier=pstate->grow_batches_barrier; 
    ...
    oldcxt = MemoryContextSwitchTo(hashtable->hashCxt);
    pthread_spin_lock(hashtable->parallel_state->chunkspinlock);
    if (hashtable->innerBatchFile == NULL) {
        /* we had no file arrays before */
        hashtable->innerBatchFile = (BufFile**)palloc0(nbatch * sizeof(BufFile*));
        hashtable->outerBatchFile = (BufFile**)palloc0(nbatch * sizeof(BufFile*));
    ...
    pthread_spin_unlock(hashtable->parallel_state->chunkspinlock);
    MemoryContextSwitchTo(oldcxt);
    ...
    if (hashtable->nbuckets_optimal != hashtable->nbuckets) {
        ...
        if(myBarrierArriveAndWait(grow_batches_barrier)){
            pstate->shared=(std::atomic<HashJoinTupleData*>*)repalloc(pstate->shared,hashtable->nbuckets * sizeof(std::atomic<HashJoinTupleData*>));
        }
    }
     ...
     */
    if(myBarrierArriveAndWait(grow_batches_barrier)){//may_dead_lock
        rc = memset_s(pstate->shared,
        hashtable->nbuckets * sizeof(std::atomic<HashJoinTupleData*>),
        0,
        hashtable->nbuckets * sizeof(std::atomic<HashJoinTupleData*>));
        securec_check(rc, "\0", "\0");

        hashtable->parallel_state->paralleloldchunks=hashtable->parallel_state->parallelchunks;
        hashtable->parallel_state->parallelchunks=NULL;
    }
    myBarrierArriveAndWait(grow_batches_barrier);

    hashtable->buckets.shared=pstate->shared;
    pstate->nbuckets=hashtable->nbuckets;
    pstate->log2_nbuckets=hashtable->log2_nbuckets;
    pstate->nbatch = nbatch;
    
    /* so, let's scan through the old chunks, and all tuples in each chunk */
    while (hashtable->parallel_state->paralleloldchunks != NULL) {
        pthread_spin_lock(hashtable->parallel_state->chunkspinlock);
        if(hashtable->parallel_state->paralleloldchunks == NULL){
            pthread_spin_unlock(hashtable->parallel_state->chunkspinlock);
            break;
        }
        oldchunks=hashtable->parallel_state->paralleloldchunks;
        hashtable->parallel_state->paralleloldchunks=oldchunks->next;
        pthread_spin_unlock(hashtable->parallel_state->chunkspinlock);
        ...
        while (idx < oldchunks->used) {
            ...
            if (batchno == curbatch) {
                ...
                pthread_spin_lock(hashtable->parallel_state->memoryspinlock);
                HashJoinTuple copyTuple = (HashJoinTuple)parallel_dense_alloc(hashtable, hashTupleSize);
                pthread_spin_unlock(hashtable->parallel_state->memoryspinlock);
                rc = memcpy_s(copyTuple, hashTupleSize, hashTuple, hashTupleSize);
                securec_check(rc, "\0", "\0");

                /* and add it back to the appropriate bucket */
                HashJoinTuple oldHead = hashtable->buckets.shared[bucketno].load();
                do {
                    copyTuple->next.shared = oldHead;
                } while (!hashtable->buckets.shared[bucketno].compare_exchange_weak(oldHead, copyTuple));
            } else {
                /* dump it out */
                Assert(batchno > curbatch);
                ExecHashJoinSaveTuple(
                    HJTUPLE_MINTUPLE(hashTuple), hashTuple->hashvalue, &hashtable->innerBatchFile[batchno]);
                hashtable->parallel_state->spaceUsed -= hashTupleSize;
                nfreed++;
            }
...
static void ExecHashIncreaseNumBatches(HashJoinTable hashtable)
{
    ...
    if (hashtable->nbuckets_optimal != hashtable->nbuckets) {
        ...
        hashtable->buckets.unshare =
            (struct HashJoinTupleData **)repalloc(hashtable->buckets.unshare, sizeof(HashJoinTuple) * hashtable->nbuckets);
    }
     ...
     * already been processed. We will free the old chunks as we go.
     */
    rc = memset_s(hashtable->buckets.unshare,
    ...
    while (oldchunks != NULL) {
        ...
        while (idx < oldchunks->used) {
            ...
            if (batchno == curbatch) {
                ...
                /* and add it back to the appropriate bucket */
                copyTuple->next.unshared = hashtable->buckets.unshare[bucketno];
                hashtable->buckets.unshare[bucketno] = copyTuple;
            } else {
                /* dump it out */
                Assert(batchno > curbatch);
                ExecHashJoinSaveTuple(
...
static void ExecParallelHashIncreaseNumBuckets(HashJoinTable hashtable)
{
    ...
    hashtable->parallel_state->nbuckets =hashtable->nbuckets;
    hashtable->parallel_state->log2_nbuckets =hashtable->log2_nbuckets;
     ...
     */
    ParallelHashJoinState *pstate = hashtable->parallel_state;
	myBarrier    *grow_buckets_barrier=pstate->grow_buckets_barrier;    
    if(myBarrierArriveAndWait(grow_buckets_barrier)){
        pstate->shared=(std::atomic<HashJoinTupleData*>*)repalloc(pstate->shared,hashtable->nbuckets* sizeof(std::atomic<HashJoinTupleData*>));
                    chunk=hashtable->parallel_state->parallelchunks;
        rc = memset_s(pstate->shared,
        hashtable->nbuckets *sizeof(std::atomic<HashJoinTupleData*>),//sizeof(void *)
        0,
        hashtable->nbuckets *sizeof(std::atomic<HashJoinTupleData*>));
        securec_check(rc, "\0", "\0");     
    }
    hashtable->parallel_state->paralleloldchunks=hashtable->parallel_state->parallelchunks;
    myBarrierArriveAndWait(grow_buckets_barrier);
    hashtable->buckets.shared = pstate->shared;
    ...
    for(;;){
        ...
        chunk=hashtable->parallel_state->paralleloldchunks;
        hashtable->parallel_state->paralleloldchunks=chunk->next;
        pthread_spin_unlock(hashtable->parallel_state->chunkspinlock);
        ...
        while (idx < chunk->used) {
            ...
            HashJoinTuple oldHead = hashtable->buckets.shared[bucketno].load();
            do {
                hashTuple->next.shared = oldHead;
            } while (!hashtable->buckets.shared[bucketno].compare_exchange_weak(oldHead, hashTuple));
...
static void ExecHashIncreaseNumBuckets(HashJoinTable hashtable)
{
     ...
     * copying into new chunks)
     */
    hashtable->buckets.unshare = (HashJoinTuple *)repalloc(hashtable->buckets.unshare, hashtable->nbuckets * sizeof(HashJoinTuple));

    rc = memset_s(hashtable->buckets.unshare, sizeof(void *) * hashtable->nbuckets, 0, sizeof(void *) * hashtable->nbuckets);
    securec_check(rc, "\0", "\0");
    ...
    for (chunk = hashtable->chunks; chunk != NULL; chunk = chunk->next) {
        ...
        while (idx < chunk->used) {
            ...
            /* add the tuple to the proper bucket */
            hashTuple->next.unshared = hashtable->buckets.unshare[bucketno];
            hashtable->buckets.unshare[bucketno] = hashTuple;
 ...
 */
void ExecParallelHashTableInsertCurrentBatch(HashJoinTable hashtable, TupleTableSlot *slot, uint32 hashvalue, int planid, int dop,
                         Instrumentation *instrument)
{
    ...
    if (batchno == hashtable->curbatch) {
        ...
        int hashTupleSize;
        double ntuples = (hashtable->parallel_state->totalTuples - hashtable->skewTuples);

        /* Create the HashJoinTuple */
        hashTupleSize = HJTUPLE_OVERHEAD + tuple->t_len;

        pthread_spin_lock(hashtable->parallel_state->memoryspinlock);
        hashTuple = (HashJoinTuple)parallel_dense_alloc(hashtable, hashTupleSize);
        pthread_spin_unlock(hashtable->parallel_state->memoryspinlock);
...
void ExecParallelHashTableInsert(HashJoinTable hashtable, TupleTableSlot *slot, uint32 hashvalue, int planid, int dop,
                         Instrumentation *instrument)
{
    MinimalTuple tuple = ExecFetchSlotMinimalTuple(slot);//获取一个MinimalTuple
    int bucketno;
    int batchno;
    errno_t errorno = EOK;

    ExecHashGetBucketAndBatch(hashtable, hashvalue, &bucketno, &batchno);//获取桶号和批次号
    ...
    if (batchno == hashtable->curbatch) {
        ...
        hashtable->parallel_state->spaceUsed += hashTupleSize;
        if (hashtable->parallel_state->spaceUsed > hashtable->spacePeak) {
            hashtable->spacePeak = hashtable->parallel_state->spaceUsed;//超出峰值,则跳转
        }
        bool sysBusy = gs_sysmemory_busy(hashtable->parallel_state->spaceUsed * dop, false);


        if (hashtable->parallel_state->spaceUsed + int64(hashtable->nbuckets_optimal * sizeof(HashJoinTuple)) >
                hashtable->spaceAllowed ||sysBusy) {
            AllocSetContext *set = (AllocSetContext *)(hashtable->hashCxt);
            if (sysBusy) {
                hashtable->causedBySysRes = true;
                hashtable->spaceAllowed = hashtable->parallel_state->spaceUsed;
                set->maxSpaceSize = hashtable->parallel_state->spaceUsed;
                ...
                if (hashtable->growEnabled) {
                    MEMCTL_LOG(LOG, "HashJoin(%d) early spilled, workmem: %ldKB, usedmem: %ldKB", planid,
                               hashtable->spaceAllowed / 1024L, hashtable->parallel_state->spaceUsed / 1024L);
                ...
                if (spreadMem > hashtable->spaceAllowed * MEM_AUTO_SPREAD_MIN_RATIO) {
                    ...
                    hashtable->spreadNum++;
                    ExecParallelHashIncreaseBuckets(hashtable);
            ...
            if (instrument != NULL) {
                instrument->memoryinfo.peakOpMemory = hashtable->parallel_state->spaceUsed;
            }
...
{
    List* hjclauses = hjstate->hashclauses;//hash连接条件表达式
 ...
 */
    if (hashTuple != NULL)
        hashTuple = hashTuple->next.shared;
    else if (hjstate->hj_CurSkewBucketNo != INVALID_SKEW_BUCKET_NO)
        hashTuple = hashtable->skewBucket[hjstate->hj_CurSkewBucketNo]->tuples;
    else
        hashTuple = hashtable->buckets.shared[hjstate->hj_CurBucketNo];

    while (hashTuple != NULL) {
        if (hashTuple->hashvalue == hashvalue) {
            TupleTableSlot* inntuple = NULL;//inner tuple
...
bool ExecScanHashBucket(HashJoinState* hjstate, ExprContext* econtext)
{
    ...
    if (hashTuple != NULL)
        ...
        hashTuple = hashtable->buckets.unshare[hjstate->hj_CurBucketNo];
...
void ExecHashTableReset(HashJoinTable hashtable)
{
    if(hashtable->parallel_state){
        ...
        */
        MemoryContextReset(hashtable->parallel_state->sharebatchCxt);
        hashtable->sharebatchCxt=hashtable->parallel_state->sharebatchCxt;
        oldcxt = MemoryContextSwitchTo(hashtable->sharebatchCxt);

        /* Reallocate and reinitialize the hash bucket headers. */
        hashtable->parallel_state->shared = (std::atomic<HashJoinTupleData*>*)palloc0(nbuckets * sizeof(std::atomic<HashJoinTupleData*>));
        for(int i=0;i<nbuckets;i++){
            hashtable->parallel_state->shared[i]=nullptr;
        }

        hashtable->parallel_state->spaceUsed = 0;
        //hashtable->spaceUsed = 0;
    ...
    /* Reallocate and reinitialize the hash bucket headers. */
        hashtable->buckets.unshare = (HashJoinTuple*)palloc0(nbuckets * sizeof(HashJoinTuple));
...
void ExecHashTableResetMatchFlags(HashJoinTable hashtable)
{
    ...
    for (i = 0; i < hashtable->nbuckets; i++) {
        for (tuple = hashtable->buckets.unshare[i]; tuple != NULL; tuple = tuple->next.unshared)
...
static void ExecParallelHashIncreaseBuckets(HashJoinTable hashtable)
{
    ...
    if (hashtable->parallel_state->totalTuples * 2 <= hashtable->nbuckets)
        return;
	...
	myBarrier    *grow_buckets_barrier=pstate->grow_buckets_barrier;    
    if(myBarrierArriveAndWait(grow_buckets_barrier)){
        pstate->shared=(std::atomic<HashJoinTupleData*>*)repalloc(pstate->shared,hashtable->nbuckets* 2 * sizeof(std::atomic<HashJoinTupleData*>));
        rc = memset_s(pstate->shared + hashtable->nbuckets,
        hashtable->nbuckets * sizeof(std::atomic<HashJoinTupleData*>),
        0,
        hashtable->nbuckets * sizeof(std::atomic<HashJoinTupleData*>));
        securec_check(rc, "\0", "\0");
     ...
     * no longer of the current batch.
     */
    int segment=hashtable->nbuckets/hashtable->producer_dop;
    int start=(segment+1)*hashtable->smp_id;
    int end=start+segment+1;
    if(end>hashtable->nbuckets)
        end=hashtable->nbuckets;
    //for (int i = 0; i < hashtable->nbuckets; i++) {
    for (int i = start; i < end; i++) {
        HashJoinTuple htuple = hashtable->buckets.shared[i];
        ...
        while (htuple != NULL) {
            ...
            ntotal++;
            next = htuple->next.shared;
            if (offset == 1) {
                if (prev == NULL)
                    hashtable->buckets.shared[i] = htuple->next.shared;
                else
                    prev->next.shared = htuple->next.shared;

                HashJoinTuple oldHead = hashtable->buckets.shared[i + hashtable->nbuckets].load();
                do {
                    htuple->next.shared = oldHead;
                } while (!hashtable->buckets.shared[i + hashtable->nbuckets].compare_exchange_weak(oldHead, htuple));
...
static void ExecHashIncreaseBuckets(HashJoinTable hashtable)
{
    ...
    hashtable->buckets.unshare = (HashJoinTuple*)repalloc(hashtable->buckets.unshare, hashtable->nbuckets * 2 * sizeof(HashJoinTuple));
    rc = memset_s(hashtable->buckets.unshare + hashtable->nbuckets,
     ...
     */
    for (int i = 0; i < hashtable->nbuckets; i++) {
        HashJoinTuple htuple = hashtable->buckets.unshare[i];
        HashJoinTuple prev = NULL;
        HashJoinTuple next = NULL;
        while (htuple != NULL) {
            int offset = (htuple->hashvalue >> hashtable->log2_nbuckets) & 1;
            ...
            if (offset == 1) {
                if (prev == NULL)
                    hashtable->buckets.unshare[i] = htuple->next.unshared;
                else
                    prev->next.unshared = htuple->next.unshared;
                htuple->next.unshared = hashtable->buckets.unshare[i + hashtable->nbuckets];
                hashtable->buckets.unshare[i + hashtable->nbuckets] = htuple;
                Assert((int32)(htuple->hashvalue % (hashtable->nbuckets * 2)) == i + hashtable->nbuckets);
                nmove++;
            } else {
                prev = htuple;
...
static void* parallel_dense_alloc(HashJoinTable hashtable, Size size)
{
    //hashtable->parallelchunks=hashtable->parallel_state->parallelchunks;
    ...
    if (size > HASH_CHUNK_THRESHOLD) {
        ...
        newChunk = (HashMemoryChunk)MemoryContextAlloc(hashtable->sharebatchCxt, offsetof(HashMemoryChunkData, data) + size);
         ...
         */
        if (hashtable->parallel_state->parallelchunks != NULL) {
            newChunk->next = hashtable->parallel_state->parallelchunks->next;
            hashtable->parallel_state->parallelchunks->next = newChunk;
        } else {
            newChunk->next = hashtable->parallel_state->parallelchunks;
            hashtable->parallel_state->parallelchunks = newChunk;
        }
     ...
     */
    if ((hashtable->parallel_state->parallelchunks == NULL) || (hashtable->parallel_state->parallelchunks->maxlen - hashtable->parallel_state->parallelchunks->used) < size) {
        /* allocate new chunk and put it at the beginning of the list */
        newChunk = (HashMemoryChunk)MemoryContextAlloc(
            hashtable->sharebatchCxt, offsetof(HashMemoryChunkData, data) + HASH_CHUNK_SIZE);
    ...
    ptr = hashtable->parallel_state->parallelchunks->data + hashtable->parallel_state->parallelchunks->used;
    hashtable->parallel_state->parallelchunks->used += size;
    hashtable->parallel_state->parallelchunks->ntuples += 1;
