...
static TupleTableSlot* ExecHashJoin(PlanState* state)
{
    ...
    JoinType jointype;
    ParallelHashJoinState *parallel_state=node->parallel_state;
        ...
        switch (node->hj_JoinState) {
            case HJ_BUILD_HASHTABLE: {
                ...
                hashtable = ExecHashTableCreate((Hash*)hashNode->ps.plan, node->hj_HashOperators,
                    HJ_FILL_INNER(node) || node->js.nulleqqual != NIL, node->hj_hashCollations,parallel_state);
                ...
                if (hashtable->totalTuples == 0 && !HJ_FILL_OUTER(node)
                    &&parallel_state&&parallel_state->totalTuples==0) {
                    /*
                ...
                if(parallel_state)
                    myBarrierArriveAndWait(parallel_state->build_barrier);
            }
            /* fall through */
            case HJ_NEED_NEW_OUTER:
                ...
                if (TupIsNull(outerTupleSlot)) {
                    // if(parallel_state){
                    //     myBarrier* build_barrier=parallel_state->build_barrier;
                    //     myBarrierArriveAndWait(build_barrier);//may_dead_lock
                    // }
            ...
            case HJ_SCAN_BUCKET:
                ...
                if (joinqual == NIL || ExecQual(joinqual, econtext, false)) {
                    ...
                    if (jointype == JOIN_RIGHT_ANTI || jointype == JOIN_RIGHT_SEMI ||
                        ...
                        if (node->hj_PreTuple)
                            node->hj_PreTuple->next.unshared = node->hj_CurTuple->next.unshared;
                        else if (node->hj_CurSkewBucketNo != INVALID_SKEW_BUCKET_NO)
                            hashtable->skewBucket[node->hj_CurSkewBucketNo]->tuples = node->hj_CurTuple->next.unshared;
                        else
                            hashtable->buckets.unshare[node->hj_CurBucketNo] = node->hj_CurTuple->next.unshared;
            ...
            case HJ_NEED_NEW_BATCH:
                ...
                if(parallel_state){
                    if (!ExecParallelHashJoinNewBatch(node)) {
                        myBarrierDetach(parallel_state->build_barrier,false);
                        if(parallel_state->batch_barrier->participants!=0)
                            myBarrierDetach(parallel_state->batch_barrier,false);
...
HashJoinState* ExecInitHashJoin(HashJoin* node, EState* estate, int eflags)
{
    ...
    hjstate->parallel_state=NULL;

    int smp_id=u_sess->stream_cxt.smp_id;
    int producer_dop=u_sess->stream_cxt.producer_dop;
    StreamSharedContext* StreamSharedContext;
    if(producer_dop > 1){
        StreamSharedContext=u_sess->stream_cxt.producer_obj->getSharedContext();
        hjstate->parallel_state=StreamSharedContext->parallel_state;
    }
...
static bool ExecParallelHashJoinNewBatch(HashJoinState* hjstate)
{
    ...
    myBarrier* batch_barrier=hashtable->parallel_state->batch_barrier;
    if(myBarrierArriveAndWait(batch_barrier)){
        /*
    ...
    myBarrierArriveAndWait(batch_barrier);
    hashtable->buckets.shared=hashtable->parallel_state->shared;
    ...
    if (innerFile != NULL) {
        ...
        while ((slot = ExecHashJoinGetSavedTuple(hjstate, innerFile, &hashvalue, hjstate->hj_HashTupleSlot))) {
             ...
             */
            ExecParallelHashTableInsertCurrentBatch(hashtable,
                slot,
                hashvalue,
