...
typedef struct HashJoinTupleData {
    union{
        struct HashJoinTupleData* unshared; /* link to next tuple in same bucket */
        struct HashJoinTupleData* shared;
    }next;
...
typedef struct HashJoinTableData {
    ...
    union{
        ...
        struct HashJoinTupleData** unshare;
        /* buckets array is per-batch storage, as are all the tuples */
        std::atomic<struct HashJoinTupleData*>* shared;
    }buckets;
    ...
    HashMemoryChunk parallelchunks;
    HashMemoryChunk paralleloldchunks;
    MemoryContext sharebatchCxt;
    int smp_id;
    int producer_dop;
} HashJoinTableData;
...
typedef struct ParallelHashJoinState
{
	...
	myBarrier*		build_barrier;	/* synchronization for the build phases */
    myBarrier*      batch_barrier;

    pthread_spinlock_t* memoryspinlock;
    pthread_spinlock_t* chunkspinlock;
    //pthread_spinlock_t* rwspinlock;
    HashMemoryChunk parallelchunks;
    HashMemoryChunk paralleloldchunks;
    MemoryContext sharebatchCxt;
    std::atomic<struct HashJoinTupleData*>* shared;
    ...
    std::atomic<int64> spaceUsed;
    std::atomic<int64> totalTuples;
    int nbuckets;      
    int log2_nbuckets;
    int nbatch;
