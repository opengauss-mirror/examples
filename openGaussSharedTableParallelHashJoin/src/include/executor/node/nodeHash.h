...
extern Node* MultiExecPrivateHash(HashState* node);
extern Node* MultiExecParallelHash(HashState* node);
extern void ExecParallelHashTableInsert(HashJoinTable hashtable, TupleTableSlot* slot, uint32 hashvalue, int planid, int dop,
    Instrumentation* instrument = NULL);
extern void ExecParallelHashTableInsertCurrentBatch(HashJoinTable hashtable, TupleTableSlot* slot, uint32 hashvalue, int planid, int dop,
Instrumentation* instrument = NULL);
extern bool ExecParallelScanHashBucket(HashJoinState* hjstate, ExprContext* econtext);
extern HashJoinTable ExecHashTableCreate(Hash* node, List* hashOperators, bool keepNulls, List *hash_collations,ParallelHashJoinState* parallel_state);
extern void ExecParallelHashIncreaseNumBatches(HashJoinTable hashtable);
#endif /* NODEHASH_H */
