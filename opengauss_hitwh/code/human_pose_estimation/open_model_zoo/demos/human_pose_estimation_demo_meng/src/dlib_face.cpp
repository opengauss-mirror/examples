...
int eye_func(cv::Mat temp, dlib::shape_predictor pos_model, dlib::frontal_face_detector detector)
{
    ...
    std::cout << "shapes_human_demo_meng: " << shapes.size() << std::endl;
    if (!shapes.empty())
    {
        //68个关键点的描绘
        ...
        int height_right_eye1 = y_47 - y_43;                                  //37到41的纵向距离
        ...
        std::string res = Convert(EAR_left_eye);
        res.append(",");
