...
static DMODBC::ProcOdbc *sg_procOdbc;
static DMODBC::ProcMetrics *sg_procMetrics;
static struct DMODBC::MysqlInfo sg_mysqlInfo;
static std::map<std::string, std::string> sg_createSuccessTables;
static std::list<std::string> sg_createFailedTables;
static bool sg_firstdumptables;
static pthread_key_t sg_connectionkey;
// static std::map<std::string, std::list<std::string> >
// sg_createFailedTableSqls;
...
my_bool opt_force = 0;
my_bool opt_comments = 1;
my_bool opt_comments_used = 1;
char *path = 0;
my_bool opt_xml = 0;
...
class MySQLContext {
 public:
  MySQLContext();
  void init_mysql_context();
  void write_header(FILE *sql_file, char *db_name);
  void write_footer(FILE *sql_file);
  my_bool get_one_option(int optid, const char *argument);
  int get_options(int *argc, char ***argv);
  int fetch_db_collation(const char *db_name, char *db_cl_name, int db_cl_size);
  void free_resources();
  int mainproc(int argc, char **argv);
  int switch_character_set_results(MYSQL *mysql, const char *cs_name);
  int connect_to_db(char *host, char *user, char *passwd);
  my_bool do_ignore_error();
  void maybe_exit(int error);
  void dbDisconnect(char *host);
  void unescape(FILE *file, char *pos, size_t length, DYNAMIC_STRING *output);
  void unescapestr(char *pos, size_t length, DYNAMIC_STRING *output);
  ...
  void dump_trigger_old(FILE *sql_file, MYSQL_RES *show_triggers_rs,
                        MYSQL_ROW *show_trigger_row, const char *table_name);
  ...
  int dump_tablespaces_for_tables(char *db, char **table_names, int tables);
  int dump_tablespaces(char *ts_where);
  int is_ndbinfo(MYSQL *mysql, const char *dbname);
  int dump_all_databases();
  int dump_databases(char **db_names);
  int init_dumping(char *database, int init_func);
  ...
  char *get_actual_table_name(const char *old_table_name, MEM_ROOT *root);
  int dump_selected_tables(char *db, char **table_names, int tables);
  int dump_tablespaces_for_databases(char **databases);
  int switch_db_collation(FILE *sql_file, const char *db_name,
                          const char *delimiter, const char *current_db_cl_name,
                          const char *required_db_cl_name, int *db_cl_altered);
  int restore_db_collation(FILE *sql_file, const char *db_name,
  ...
  int do_show_master_status(MYSQL *mysql_con);
  int do_stop_slave_sql(MYSQL *mysql_con);
  int add_stop_slave(void);
  int add_slave_statements(void);
  int do_show_slave_status(MYSQL *mysql_con);
  int do_start_slave_sql(MYSQL *mysql_con);
  int do_flush_tables_read_lock(MYSQL *mysql_con);
  int do_unlock_tables(MYSQL *mysql_con);
  int get_bin_log_name(MYSQL *mysql_con, char *buff_log_name, uint buff_len);
  int purge_bin_logs_to(MYSQL *mysql_con, char *log_name);
  int start_transaction(MYSQL *mysql_con);
  ulong find_set(TYPELIB *lib, const char *x, size_t length, char **err_pos,
                 uint *err_len);
  void print_value(FILE *file, MYSQL_RES *result, MYSQL_ROW row,
                   const char *prefix, const char *name, int string_value);
  my_bool process_set_gtid_purged(MYSQL *mysql_con);
 ...
 private:
  ...
  my_bool lock_tables;
  my_bool flush_logs;
  my_bool flush_privileges;
  my_bool opt_drop;
  my_bool opt_keywords;
  my_bool opt_lock;
  my_bool opt_compress;
  my_bool create_options;
  my_bool opt_quoted;
  my_bool opt_databases;
  my_bool opt_alldbs;
  my_bool opt_create_db;
  my_bool opt_lock_all_tables;
  my_bool opt_set_charset;
  my_bool opt_dump_date;
  my_bool opt_autocommit;
  my_bool opt_disable_keys;
  my_bool opt_delete_master_logs;
  my_bool tty_password;
  my_bool opt_single_transaction;
  my_bool opt_compact;
  my_bool opt_hex_blob;
  my_bool opt_order_by_primary;
  my_bool opt_ignore;
  my_bool opt_complete_insert;
  my_bool opt_drop_database;
  my_bool opt_replace_into;
  my_bool opt_dump_triggers;
  my_bool opt_routines;
  my_bool opt_tz_utc;
  my_bool opt_slave_apply;
  my_bool opt_include_master_host_port;
  my_bool opt_events;
  my_bool opt_alltspcs;
  my_bool opt_notspcs;
  my_bool opt_drop_trigger;
  my_bool opt_secure_auth;
  my_bool insert_pat_inited;
  my_bool debug_info_flag;
  my_bool debug_check_flag;
  ...
  char *current_user;
  char *current_host;
  char *fields_terminated;
  char *lines_terminated;
  char *enclosed;
  char *opt_enclosed;
  char *escaped;
  char *where;
  char *order_by;
  char *opt_compatible_mode_str;
  char *err_ptr;
  char *log_error_file;
  char **defaults_argv;
  char compatible_mode_normal_str[255];
  /* Server supports character_set_results session variable? */
  my_bool server_supports_switching_charsets;
  ulong opt_compatible_mode;
  uint opt_enable_cleartext_plugin;
  my_bool using_opt_enable_cleartext_plugin;
  uint opt_mysql_port;
  uint opt_master_data;
  uint opt_slave_data;
  uint my_end_arg;
  char *opt_mysql_unix_port;
  char *opt_bind_addr;
  int first_error;
};

MySQLContext::MySQLContext() { init_mysql_context(); }

void MySQLContext::init_mysql_context() {
  verbose = 0;
  opt_no_create_info = 0;
  opt_no_data = 0;
  quick = 1;
  extended_insert = 1;
  lock_tables = 1;
  ...
  debug_info_flag = 0;
  debug_check_flag = 0;
  opt_max_allowed_packet = 24*1024*1024;
  opt_net_buffer_length = 1024*1024L-1025;
  mysql = 0;
  opt_password = 0;
  current_user = 0;
  current_host = 0;
  ...
  log_error_file = NULL;
  defaults_argv = 0;
  server_supports_switching_charsets = TRUE;
  opt_compatible_mode = 0;
  opt_enable_cleartext_plugin = 0;
  using_opt_enable_cleartext_plugin = 0;
  opt_mysql_port = 0;
  opt_master_data = 0;
  opt_slave_data = 0;
  my_end_arg = 0;
...
static MySQLContext *create_connection_thread_context() {
  MySQLContext *ret = new MySQLContext();
  pthread_setspecific(sg_connectionkey, ret);
  return ret;
}

static MySQLContext *get_connection_thread_context() {
  return (MySQLContext *)pthread_getspecific(sg_connectionkey);
}

static void release_connection_thread_context() {
  MySQLContext *context = (MySQLContext *)pthread_getspecific(sg_connectionkey);
  if (context) {
    delete context;
  }
}
...
*/
static const char *mysql_universal_client_charset = "utf8";
...
void MySQLContext::write_header(FILE *sql_file, char *db_name) {
  if (opt_xml) {
...
void MySQLContext::write_footer(FILE *sql_file) {
  if (opt_xml) {
    fputs("</mysqldump>\n", sql_file);
    check_io(sql_file);
  } else if (!opt_compact) {
...
my_bool MySQLContext::get_one_option(int optid, const char *argument) {
  switch (optid) {
    case (int)OPT_COMPATIBLE: {
      ...
      opt_quoted = 1;
      opt_set_charset = 1;
      opt_compatible_mode_str = (char *)argument;
...
int MySQLContext::get_options(int *argc, char ***argv) {
  ...
  if (my_hash_insert(&ignore_table,
      ...
      my_hash_insert(&ignore_table,
                     (uchar *)my_strdup(PSI_NOT_INSTRUMENTED, "mysql.slow_log",
                                        MYF(MY_WME))))
    return (EX_EOM);

  get_one_option(OPT_COMPATIBLE, "postgresql");
...
static void die(int error_num, const char *fmt_reason, ...) {
  ...
  MySQLContext *context = get_connection_thread_context();
  context->maybe_exit(error_num);
}
...
static int mysql_query_without_exit(MYSQL *mysql_con, MYSQL_RES **res,
                                    const char *query) {
  if (mysql_query(mysql_con, query) ||
      (res && !((*res) = mysql_store_result(mysql_con)))) {
    printf("Couldn't execute '%s': %s (%d)\n", query, mysql_error(mysql_con),
...
int MySQLContext::fetch_db_collation(const char *db_name, char *db_cl_name,
                                     int db_cl_size) {
...
static char *my_case_str(const char *str, size_t str_len, const char *token,
  ...
  uint status = my_charset_latin1.coll->instr(&my_charset_latin1, str, str_len,
                                              token, token_len, &match, 1);

  return status ? (char *)str + match.end : NULL;
}

int MySQLContext::switch_db_collation(FILE *sql_file, const char *db_name,
...
int MySQLContext::restore_db_collation(FILE *sql_file, const char *db_name,
                                       const char *delimiter,
...
*/
int MySQLContext::switch_character_set_results(MYSQL *mysql,
                                               const char *cs_name) {
...
void MySQLContext::free_resources() {
  ...
  my_end(my_end_arg);
  pthread_key_delete(sg_connectionkey);
}
...
static int parse_ignore_error() {
  ...
  if (my_init_dynamic_array(&ignore_error, PSI_NOT_INSTRUMENTED, sizeof(uint),
                            NULL, 12, 12))
    goto error;
  ...
  while (token != NULL) {
    ...
    if (my_err != 0) {
      if (insert_dynamic(&ignore_error, &my_err)) goto error;
    }
...
/**
  Check if the last error should be ignored.
      @retval 1     yes
              0     no
*/
my_bool MySQLContext::do_ignore_error() {
...
void MySQLContext::maybe_exit(int error) {
...
int MySQLContext::connect_to_db(char *host, char *user, char *passwd) {
  ...
  */
  ...
  my_snprintf(buff, sizeof(buff), "/*!40100 SET @@SQL_MODE='%s' */",
              compatible_mode_normal_str);
  if (mysql_query_without_exit(mysql, 0, buff)) {
    printf("cannot set sql_mode to %s, set sql_mode to ansi and retry",
           compatible_mode_normal_str);
    my_snprintf(buff, sizeof(buff), "/*!40100 SET @@SQL_MODE='ANSI' */");
...
*/
void MySQLContext::dbDisconnect(char *host) {
  verbose_msg("-- Disconnecting from %s...\n", host ? host : "localhost");
  mysql_close(mysql);
} /* dbDisconnect */

void MySQLContext::unescape(FILE *file, char *pos, size_t length,
  ...
  fputc('\'', file);
  if (output) {
    dynstr_append_checked(output, "\'");
    dynstr_append_checked(output, tmp);
    dynstr_append_checked(output, "\'");
  }
  check_io(file);
  my_free(tmp);
...
void MySQLContext::unescapestr(char *pos, size_t length,
                               DYNAMIC_STRING *output) {
  char *tmp;
  DBUG_ENTER("unescape");
  if (!(tmp = (char *)my_malloc(PSI_NOT_INSTRUMENTED, length * 2 + 1,
...
*/
char *MySQLContext::quote_name(const char *name, char *buff, my_bool force) {
  char *to = buff;
...
/*
  ...
  RETURN
    ...
    1  Error
*/
uint MySQLContext::dump_events_for_db(char *db) {
...
static void append_blob_as_hex(DYNAMIC_STRING *output, const char *str,
                               ulong len) {
  /* sakaik got the idea to to provide blob's in hex notation. */
  static char tmp[256];
  const char *ptr = str, *end = ptr + len;
  for (; ptr < end; ptr++) {
    sprintf(tmp, "%02X", *((uchar *)ptr));
    dynstr_append_checked(output, tmp);
  }
}
...
uint MySQLContext::dump_routines_for_db(char *db) {
  char query_buff[QUERY_LENGTH];
        ...
        while ((row = mysql_fetch_row(routine_res))) {
          ...
          */
          ...
          if (row[2] == NULL) {
            ...
            // remove_sql_mode(row[1],
            // C_STRING_WITH_LEN("NO_AUTO_CREATE_USER"));
            ...
            sql = sg_procOdbc->procProcedureFunctionSql((const char *)row[2],
                                                        isfunction);
            if (!sql.empty()) {
              // execute sql
              if (isfunction) {
                sg_procMetrics->addFunctioncount(1);
                sg_procMetrics->startSql("function", "function", sql.c_str());
              } else {
                sg_procMetrics->addProcedurecount(1);
                sg_procMetrics->startSql("procedure", "procedure", sql.c_str());
              }
              if (sg_procOdbc->executeGeneralSql(sql.c_str(), &dbError)) {
                // successful
                if (isfunction) {
                  sg_procMetrics->endSql("function", "function", sql.c_str(),
                                         false, "", false);
                } else {
                  sg_procMetrics->endSql("procedure", "procedure", sql.c_str(),
                                         false, "", false);
                }
              } else {
                // error
                if (isfunction) {
                  sg_procMetrics->endSql("function", "function", sql.c_str(),
                                         true, dbError.error.c_str(), false);
                } else {
                  sg_procMetrics->endSql("procedure", "procedure", sql.c_str(),
                                         true, dbError.error.c_str(), false);
                }
...
uint MySQLContext::get_table_structure(char *table, char *db, char *table_type,
                                       char *ignore_flag,
                                       my_bool real_columns[]) {
  ...
  const char *show_increment =
      "SELECT `COLUMN_NAME` AS `Field`"
      "FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE "
      "TABLE_SCHEMA = '%s' AND TABLE_NAME = '%s' AND EXTRA='auto_increment'";
  ...
  *ignore_flag = check_if_ignore_table(table, table_type);
  ...
  // 查询自增主键
  char buff4ik[20 + FN_REFLEN];
  my_snprintf(buff4ik, sizeof(buff4ik), show_increment, db, table);
  if (!mysql_query_with_error_report(mysql, &result, buff4ik)) {
  ...
  if (!opt_xml && !mysql_query_with_error_report(mysql, 0, query_buff)) {
    ...
    if (!opt_no_create_info) {
      ...
      // execute sql
      if (sg_firstdumptables) {
        sg_procMetrics->addTablecount(1);
      }
      std::list<std::string> createsqls =
          sg_procOdbc->procCreateTableSql(row[1], incrementkey.c_str());
      bool hastable = false;
      struct DMODBC::DBError dbError;
      for (std::list<std::string>::iterator iter = createsqls.begin();
           iter != createsqls.end(); iter++) {
        sg_procMetrics->startSql(table, "table", iter->c_str());
        if (sg_procOdbc->executeCreateTableSql(iter->c_str(), &dbError)) {
          // successful sqls
          sg_procMetrics->endSql(table, "table", iter->c_str(), false, "", false);
          if (!hastable) {
            if (iter->find("TABLE ") != std::string::npos) {
              sg_createSuccessTables[table] = table;
              hastable = true;
            }
          }
        } else {
          // failed sqls
          sg_procMetrics->endSql(table, "table", iter->c_str(), true,
                                 dbError.error.c_str(), false);
          if (!hastable) {
            if (iter->find("TABLE ") != std::string::npos) {
              // 创建表语句失败
              if (!dbError.alreadyExistError) {
                // 如果不是已经存在的错误，那么保存错误信息
                sg_createFailedTables.push_back(table);
                // sg_createFailedTableSqls[table] = createsqls;
                hastable = true;
              }
              break;
            }
    ...
    */
    ...
    colno = 0;
    while ((row = mysql_fetch_row(result))) {
      real_columns[colno] = strcmp(row[SHOW_EXTRA], "STORED GENERATED") != 0 &&
                            strcmp(row[SHOW_EXTRA], "VIRTUAL GENERATED") != 0;
    ...
    while ((row = mysql_fetch_row(result))) {
      ...
      if (!opt_no_create_info) {
        ...
        if (row[SHOW_DEFAULT]) {
          ...
          unescape(sql_file, row[SHOW_DEFAULT], lengths[SHOW_DEFAULT], NULL);
        }
...
void MySQLContext::dump_trigger_old(FILE *sql_file, MYSQL_RES *show_triggers_rs,
...
int MySQLContext::dump_trigger(FILE *sql_file,
...
int MySQLContext::dump_triggers_for_table(char *table_name, char *db_name) {
  char name_buff[NAME_LEN * 4 + 3];
  char query_buff[QUERY_LENGTH];
...
void MySQLContext::dump_table(char *table, char *db) {
  char ignore_flag;
  char buf[200], table_buff[NAME_LEN + 3];
  DYNAMIC_STRING query_string;
  DYNAMIC_STRING extended_row;
  DYNAMIC_STRING insert_row;
  ...
  */
  ...
  result_table = quote_name(table, table_buff, 1);
  // opt_quoted_table =
  quote_name(table, table_buff2, 0);
  ...
  init_dynamic_string_checked(&insert_row, "", 1024, 1024);
  ...
  if (path) {
    ...
    */
    ...
    if (where) {
      ...
      dynstr_append_checked(&query_string, where);
               ...
               (!my_strcasecmp(charset_info, table, "proc")) && opt_alldbs) {
    ...
    */
    ...
    while ((row = mysql_fetch_row(res))) {
      ...
      for (i = 0; i < mysql_num_fields(res); i++) {
        ...
        */
        ...
        if (extended_insert && !opt_xml) {
          ...
          if (row[i]) {
            if (length) {
              if (!(field->flags & NUM_FLAG)) {
                /*
                  "length * 2 + 4 + 1" is OK for both HEX  non-HEX modes
                */
                dynstr_realloc_checked(&extended_row, length * 2 + 4 + 1);
                if (opt_hex_blob && is_blob) {
                  ...
                  if (is_blob) {
                    dynstr_append_checked(&extended_row, "'\\x");
                    extended_row.length += mysql_hex_string(
                        extended_row.str + extended_row.length, row[i], length);
                    dynstr_append_checked(&extended_row, "'");
                  } else {
          ...
          if (i && !opt_xml) {
            dynstr_append_checked(&insert_row, ",");
          ...
          if (row[i]) {
            if (!(field->flags & NUM_FLAG)) {
              if (opt_xml) {
                ...
                dynstr_append_checked(&insert_row, "0x");
                // fputs("0x", md_result_file);
                append_blob_as_hex(&insert_row, row[i], length);
                // print_blob_as_hex(md_result_file, row[i], length);
              } else {
                if (is_blob) {
                  // dynstr_append_checked(&insert_row,"_binary ");
                  // fputs("_binary ", md_result_file);
                  // check_io(md_result_file);
                }
                // unescape(md_result_file, row[i], length, &insert_row);
                unescapestr(row[i], length, &insert_row);
              }
            } else {
              /* change any strings ("inf", "-inf", "nan") into NULL */
              char *ptr = row[i];
              if (opt_xml) {
                ...
                fputs("</field>\n", md_result_file);
                         ...
                         (*ptr == '-' && my_isalpha(charset_info, ptr[1]))) {
                dynstr_append_checked(&insert_row, "NULL ");
                // fputs("NULL", md_result_file);
              } else if (field->type == MYSQL_TYPE_DECIMAL) {
                /* add " signs around */
                dynstr_append_checked(&insert_row, "\'");
                dynstr_append_checked(&insert_row, ptr);
                dynstr_append_checked(&insert_row, "\'");
                // fputc('\'', md_result_file);
      ...
      if (extended_insert) {
        ...
        if (total_length + row_length < opt_net_buffer_length) {
          ...
          dynstr_append_checked(&insert_row, ",");
          dynstr_append_checked(&insert_row, extended_row.str);
          ...
          if (row_break) {
            ...
            if (sg_createSuccessTables.find(table) !=
                sg_createSuccessTables.end()) {
              // 如果表已经创建成功，那么插入sql
              sg_procMetrics->startSql(table, "table", insert_row.str);
              if (sg_procOdbc->executeInsertSql(insert_row.str, &dbError)) {
                // success
                sg_procMetrics->endSql(table, "table", insert_row.str, false,
                                       "", true);
              } else {
                // failed
                sg_procMetrics->endSql(table, "table", insert_row.str, true,
                                       dbError.error.c_str(), true);
              }
            }
            dynstr_set_checked(&insert_row, "");
          }
          row_break = 1; /* This is first row */

          dynstr_append_checked(&insert_row, insert_pat.str);
          dynstr_append_checked(&insert_row, extended_row.str);
          // fputs(insert_pat.str,md_result_file);
          // fputs(extended_row.str,md_result_file);
    ...
    if (strlen(insert_row.str) > 0) {
      if (sg_createSuccessTables.find(table) != sg_createSuccessTables.end()) {
        // 如果表已经创建成功，那么插入sql
        sg_procMetrics->startSql(table, "table", insert_row.str);
        if (sg_procOdbc->executeInsertSql(insert_row.str, &dbError)) {
          // success
          sg_procMetrics->endSql(table, "table", insert_row.str, false, "", true);
        } else {
          // failed
          sg_procMetrics->endSql(table, "table", insert_row.str, true,
                                 dbError.error.c_str(), true);
...
char *MySQLContext::getTableName(int reset) {
...
int MySQLContext::dump_all_tablespaces() { return dump_tablespaces(NULL); }

int MySQLContext::dump_tablespaces_for_tables(char *db, char **table_names,
                                              int tables) {
...
int MySQLContext::dump_tablespaces_for_databases(char **databases) {
  DYNAMIC_STRING where;
  int r;
  int i;
...
int MySQLContext::dump_tablespaces(char *ts_where) {
  MYSQL_ROW row;
  MYSQL_RES *tableres;
  char buf[FN_REFLEN];
...
int MySQLContext::is_ndbinfo(MYSQL *mysql, const char *dbname) {
...
int MySQLContext::dump_all_databases() {
  MYSQL_ROW row;
  MYSQL_RES *tableres;
  int result = 0;
...
int MySQLContext::dump_databases(char **db_names) {
  int result = 0;
  char **db;
...
RETURN VALUES
  ...
  1        Failure.
*/
int MySQLContext::init_dumping_views(char *qdatabase MY_ATTRIBUTE((unused))) {
  return 0;
} /* init_dumping_views */
...
int MySQLContext::init_dumping_tables(char *qdatabase) {
...
int MySQLContext::init_dumping(char *database, int init_func) {
  ...
  if (!path && !opt_xml) {
    if (opt_databases || opt_alldbs) {
      ...
      */
      ...
      switch (init_func) {
        case 1:
          init_dumping_tables(qdatabase);
          break;
        case 2:
          init_dumping_views(qdatabase);
          break;
      }
...
int MySQLContext::dump_all_tables_in_db(char *database) {
  char *table;
  uint numrows;
  ...
  *afterdot++ = '.';
  ...
  if (init_dumping(database, 1)) DBUG_RETURN(1);
  if (opt_xml)
  ...
  sg_firstdumptables = true;
  while (alltables.size() > 0) {
    for (std::list<std::string>::iterator iter = alltables.begin();
         iter != alltables.end(); iter++) {
      table = (char *)iter->c_str();
      sg_procMetrics->startTable(table);
      char *end = my_stpcpy(afterdot, table);
      if (include_table((uchar *)hash_key, end - hash_key)) {
        ...
        */
        if (opt_single_transaction &&
            mysql_get_server_version(mysql) >= 50500) {
      ...
      sg_procMetrics->endTable(table);
    }

    alltables = sg_createFailedTables;
    sg_createFailedTables = std::list<std::string>();
    sg_firstdumptables = false;
  }
...
my_bool MySQLContext::dump_all_views_in_db(char *database) {
  char *table;
  uint numrows;
  char table_buff[NAME_LEN * 2 + 3];
  char hash_key[2 * NAME_LEN + 2]; /* "db.tablename" */
  ...
  *afterdot++ = '.';
  ...
  if (init_dumping(database, 2)) return 1;
  if (opt_xml)
    print_xml_tag(md_result_file, "", "\n", "database", "name=", database,
                  NullS);
  if (lock_tables) {
...
char *MySQLContext::get_actual_table_name(const char *old_table_name,
                                          MEM_ROOT *root) {
  char *name = 0;
  MYSQL_RES *table_res;
  MYSQL_ROW row;
...
int MySQLContext::dump_selected_tables(char *db, char **table_names,
                                       int tables) {
  char table_buff[NAME_LEN * 2 + 3];
  ...
  if (init_dumping(db, 1)) DBUG_RETURN(1);
...
int MySQLContext::do_show_master_status(MYSQL *mysql_con) {
  MYSQL_ROW row;
  MYSQL_RES *master;
...
int MySQLContext::do_stop_slave_sql(MYSQL *mysql_con) {
  MYSQL_RES *slave;
...
int MySQLContext::add_stop_slave(void) {
  if (opt_comments)
    fprintf(md_result_file,
...
int MySQLContext::add_slave_statements(void) {
  if (opt_comments)
    fprintf(md_result_file,
...
int MySQLContext::do_show_slave_status(MYSQL *mysql_con) {
...
int MySQLContext::do_start_slave_sql(MYSQL *mysql_con) {
  MYSQL_RES *slave;
...
int MySQLContext::do_flush_tables_read_lock(MYSQL *mysql_con) {
  /*
...
int MySQLContext::do_unlock_tables(MYSQL *mysql_con) {
  return mysql_query_with_error_report(mysql_con, 0, "UNLOCK TABLES");
}

int MySQLContext::get_bin_log_name(MYSQL *mysql_con, char *buff_log_name,
                                   uint buff_len) {
  MYSQL_RES *res;
  MYSQL_ROW row;
...
int MySQLContext::purge_bin_logs_to(MYSQL *mysql_con, char *log_name) {
  DYNAMIC_STRING str;
  int err;
...
int MySQLContext::start_transaction(MYSQL *mysql_con) {
...
ulong MySQLContext::find_set(TYPELIB *lib, const char *x, size_t length,
                             char **err_pos, uint *err_len) {
...
void MySQLContext::print_value(FILE *file, MYSQL_RES *result, MYSQL_ROW row,
  ...
  for (; (field = mysql_fetch_field(result)); row++) {
    if (!strcmp(field->name, name)) {
      if (row[0] && row[0][0] && strcmp(row[0], "0")) /* Skip default */
      {
        ...
        fputs(prefix, file);
        if (string_value)
          unescape(file, row[0], strlen(row[0]), NULL);
        else
          fputs(row[0], file);
        check_io(file);
        return;
...
char MySQLContext::check_if_ignore_table(const char *table_name,
                                         char *table_type) {
...
char *MySQLContext::primary_key_fields(const char *table_name) {
  MYSQL_RES *res = NULL;
  MYSQL_ROW row;
  ...
  if (result_length) {
    ...
    result = (char *)my_malloc(PSI_NOT_INSTRUMENTED, result_length + 10,
                               MYF(MY_WME));
    if (!result) {
...
my_bool MySQLContext::process_set_gtid_purged(MYSQL *mysql_con) {
...
my_bool MySQLContext::get_view_structure(char *table, char *db) {
  MYSQL_RES *table_res;
  MYSQL_ROW row;
  MYSQL_FIELD *field;
  char *result_table;  //, *opt_quoted_table;
  ...
  if (mysql_query(mysql, query)) {
    ...
    */
    ...
    //         */;\n", (const char *) row[3], (const char *) row[3], (const char
    //         *) row[4], (const char *) ds_view.str);

    createviewsql = "CREATE ";
    createviewsql += ds_view.str;
    check_io(sql_file);
    mysql_free_result(table_res);
    dynstr_free(&ds_view);
  }
  ...
  // execute sql
  struct DMODBC::DBError dbError;
  sg_procMetrics->startSql(table, "view", createviewsql.c_str());
  sg_procMetrics->addViewcount(1);
  if (sg_procOdbc->executeCreateViewSql(createviewsql.c_str(), &dbError)) {
    // successful
    sg_procMetrics->endSql(table, "view", createviewsql.c_str(), false, "", false);
  } else {
    // error
    sg_procMetrics->endSql(table, "view", createviewsql.c_str(), true,
                           dbError.error.c_str(), false);
  }
...
int main(int argc, char **argv) {
  ...
  return mySQLContext.mainproc(argc, argv);
}

int MySQLContext::mainproc(int argc, char **argv) {
  sg_procOdbc = new DMODBC::ProcOdbc("opengauss");
  if (!sg_procOdbc->connect()) {
    return 1;
  }

  if (pthread_key_create(&sg_connectionkey, 0) != 0) {
    printf("cannot create connection thread local key");
    return 1;
  }

  sg_mysqlInfo = DMODBC::ProcOdbc::getMysqlInfo();
  printf("mysql addr: %s:%d\n", sg_mysqlInfo.host.c_str(), sg_mysqlInfo.port);
  printf("mysql username: %s\n", sg_mysqlInfo.username.c_str());
  printf("mysql database: %s\n", sg_mysqlInfo.database.c_str());
  sg_procOdbc->setLoglevel(sg_mysqlInfo.loglevel);
  sg_firstdumptables = true;

  char srcaddr[256];
  char dstaddr[256];
  const char *metricsDbtype = getenv("metrics_dbtype");
  if (metricsDbtype == NULL) {
    metricsDbtype = "mysql";
  }
  char metricds[256];
  sprintf(metricds, "metrics_%s", metricsDbtype);
  sprintf(srcaddr, "%s:%d", sg_mysqlInfo.host.c_str(), sg_mysqlInfo.port);
  sprintf(dstaddr, "%s:%d", sg_mysqlInfo.dsthost.c_str(), sg_mysqlInfo.dstport);
  sg_procMetrics = new DMODBC::ProcMetrics(
      metricds, srcaddr, dstaddr, sg_mysqlInfo.database.c_str(),
      sg_mysqlInfo.dstdatabase.c_str(), 1000000, metricsDbtype);
  ...
  //   exit(exit_code);
  // }
  argc = sg_procOdbc->getCurrentArgc();
  argv = sg_procOdbc->getCurrentArgv();
  get_options(&argc, &argv);
  md_result_file = stdout;
  current_host = (char *)sg_mysqlInfo.host.c_str();
  current_user = (char *)sg_mysqlInfo.username.c_str();
  opt_password = my_strdup(PSI_NOT_INSTRUMENTED, sg_mysqlInfo.password.c_str(),
                           MYF(MY_WME));
  opt_mysql_port = sg_mysqlInfo.port;
  opt_alldbs = 0;
  // default_charset = "utf8";
  // charset_info = get_charset_by_csname(default_charset,
  //                                           MY_CS_PRIMARY, MYF(MY_WME));
  // opt_compatible_mode = 2;
  // sprintf(compatible_mode_normal_str, "POSTGRESQL");
  ...
  */
  ...
  // 设置60分钟超时
  mysql_query_with_error_report(mysql, 0, "set net_read_timeout = 3600");
  mysql_query_with_error_report(mysql, 0, "set net_write_timeout = 3600");

  sg_procMetrics->startDB();
  dump_all_tables_in_db((char *)sg_mysqlInfo.database.c_str());
  dump_all_views_in_db((char *)sg_mysqlInfo.database.c_str());
  dump_routines_for_db((char *)sg_mysqlInfo.database.c_str());
  sg_procMetrics->endDB();
...
err:
  ...
  sg_procOdbc->disConnect();
  delete sg_procOdbc;
  delete sg_procMetrics;
